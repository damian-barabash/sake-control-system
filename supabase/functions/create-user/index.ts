...
//
//   moderator  — may create/delete/reset ANY user with ANY role (member|admin|moderator)
//   admin      — may only create/delete/reset CLIENTS (role 'member') of projects they OWN
//   member/anon — forbidden
...
const ASSIGNABLE_BY_MODERATOR = ['member', 'admin', 'moderator']
...
Deno.serve(async (req) => {
  ...
  // Can the caller act on `targetId`? Moderator: yes. Admin: only members of own projects.
  async function adminCanManage(targetId: string): Promise<boolean> {
    ...
    if (tp?.role !== 'member') return false // admins never touch other admins/moderators
  ...
  try {
    if (action === 'create') {
      ...
      let role = 'member'
      if (isModerator) {
        role = ASSIGNABLE_BY_MODERATOR.includes(body.role) ? body.role : 'member'
      } // admin → always 'member'
      ...
      const { data: created, error } = await admin.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        // trigger clamps signup metadata to admin|member; we set the final role below
