...
- **Алерты:** Resend (email), отправитель `office@barabashflow.pl` (сменить через
  секрет `NOTIFY_FROM` после верификации `sakecontrol.pl` в Resend)
...
3. Settings → Pages → Custom domain: `sakecontrol.pl` (файл `public/CNAME`
   уже есть, домен сохранится между деплоями).
4. DNS: для apex `sakecontrol.pl` — A-записи на IP GitHub Pages
   (185.199.108.153 / .109 / .110 / .111) + опц. CNAME `www` → `<user>.github.io`.
...
- `NOTIFY_FROM` *(опц.)* — по умолчанию `Sake Control <office@barabashflow.pl>`
  (задать `... <office@sakecontrol.pl>` после верификации домена в Resend).
- `APP_URL` *(опц.)* — по умолчанию `https://sakecontrol.pl` (ссылка в письме).
